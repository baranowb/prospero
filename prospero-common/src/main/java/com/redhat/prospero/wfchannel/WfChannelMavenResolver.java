...
public class WfChannelMavenResolver implements MavenVersionsResolver {
    ...
    WfChannelMavenResolver(List<MavenRepository> mavenRepositories, boolean resolveLocalCache, RepositoryManager repositoryManager) {
        this.repositoryManager = repositoryManager;
        remoteRepositories = mavenRepositories.stream().map(r -> newRemoteRepository(r)).collect(Collectors.toList());
        system = repositoryManager.newRepositorySystem();
        session = repositoryManager.newRepositorySystemSession(system, resolveLocalCache);
    }
